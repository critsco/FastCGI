...
void create_json_array(Report info) {
    ...
    json_object_set_new(obj, "ticket_desc", json_string(info.ticket_desc));
...
void loadJSON(Report *info) {  
    ...
    const char* properties[] = { "id", "name", "ticket", "ticket_desc" };
    char** infovar[] = {&info->id,&info->name,&info->ticket,&info->ticket_desc};
...
int main(void) {
  ...
  while (FCGI_Accept() >= 0) {
   ...
   loadJSON(&info);
    ...
    dbres |= sqlite3_bind_text(stmt, 4, info.ticket_desc, -1, SQLITE_TRANSIENT);
